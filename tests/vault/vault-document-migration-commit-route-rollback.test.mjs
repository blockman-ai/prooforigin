...
test("migration commit rolls back copied live object when atomic promotion fails", async (t) => {
  ...
  mock.module("../../app/lib/vaultAdmin.js", {
    exports: {
      ...
      verifyVaultCiphertextObject: async () => ({ ok: true, actualBytes: 2048, actualSha256: SHA256 }),
      copyVaultStorageObject: async () => ({ error: null }),
      deleteVaultStorageObject: async (pathOrOptions) => {
        deletedPath =
          typeof pathOrOptions === "string" ? pathOrOptions : pathOrOptions?.storagePath || null;
        return { error: null };
      },
  ...
  assert.equal(json.code, "MIGRATION_COMMIT_FAILED");
  assert.equal(deletedPath, LIVE_PATH);
  assert.equal(failedReason, "commit_failed");
