...
test("migration planning rejects bound-device vault mismatch", async (t) => {
  ...
  mock.module("../../app/lib/vaultAdmin.js", {
    exports: {
      ...
      hasVerifiedVaultOwnershipForDevice: async () => {
        throw new Error("verification lookup must not run after vault mismatch");
      },
      getVaultDocumentById: async () => {
        throw new Error("source lookup must not run after vault mismatch");
      },
  ...
  assert.equal(json.code, "VAULT_MISMATCH");
