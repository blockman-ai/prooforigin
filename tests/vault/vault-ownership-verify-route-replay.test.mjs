...
test("ownership verify route rejects replayed challenge", async (t) => {
  const counters = [];
  ...
  const response = await POST(
    new Request("http://localhost/api/vault/ownership/verify", {
      ...
      body: JSON.stringify({
        ...
        challenge_nonce: "abc",
        signature: "abc",
        challenge: { challenge_type: "migration_authority_verify", vault_id: "x" },
      }),
    })
  );
  ...
  assert.equal(counters.includes("vault.ownership.verify.replay_rejected_total"), true);
