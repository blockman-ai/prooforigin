...
test("migration discovery returns safe empty result for unbound device", async (t) => {
  ...
  assert.equal(json.discovery.vault_id, null);
  assert.equal(json.discovery.documents.length, 0);
  assert.equal(json.discovery.legacy_unbound_candidate_count, 0);
  ...
  assert.equal(counterCalls.includes("vault.migration.discovery.unbound_device_total"), true);
  assert.equal(counterCalls.includes("vault.migration.discovery.success_total"), false);
