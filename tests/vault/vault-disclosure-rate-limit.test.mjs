...
test("disclosure accept rate limit blocks burst attempts and locks out repeated failures", async () => {
  resetDisclosureRateLimitsForTests();
  const req = mockReq();

  for (let i = 0; i < DISCLOSURE_ACCEPT_IP_BURST_LIMIT; i += 1) {
    const allowed = await checkDisclosureAcceptRateLimit(req, HANDLE_HASH);
    assert.equal(allowed.allowed, true, `attempt ${i + 1} should be allowed`);
  }

  const blocked = await checkDisclosureAcceptRateLimit(req, HANDLE_HASH);
  assert.equal(blocked.allowed, false);
  assert.equal(blocked.reason, "rate_limited");

  resetDisclosureRateLimitsForTests();
  for (let i = 0; i < DISCLOSURE_FAILURE_LOCKOUT_THRESHOLD; i += 1) {
    await recordDisclosureRecipientFailure(req, HANDLE_HASH);
  }

  const lockout = await checkDisclosureRecipientLockout(req, HANDLE_HASH);
  assert.equal(lockout.locked, true);
  assert.ok(lockout.retryAfterMs > 0);

  const lockedAccept = await checkDisclosureAcceptRateLimit(req, HANDLE_HASH);
