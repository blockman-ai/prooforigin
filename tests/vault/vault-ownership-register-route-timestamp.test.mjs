...
test("ownership register route accepts signatures when DB timestamps differ in format", async (t) => {
  ...
  const clientIssuedAt = "2026-06-14T17:00:00.000Z";
  const clientExpiresAt = "2099-06-14T17:05:00.000Z";
  ...
  const signature = Buffer.from(new Uint8Array(signatureBuffer)).toString("base64");
    ...
    exports: {
      ...
      getVaultOwnershipVerificationChallengeById: async () => ({
        verification: {
          ...
          issued_at: "2026-06-14 17:00:00+00",
          expires_at: "2099-06-14 17:05:00+00",
          status: "pending",
          consumed_at: null,
          vault_id: VAULT_ID,
    ...
    new Request("http://localhost/api/vault/ownership/register", {
      ...
      body: JSON.stringify({
        ...
        challenge: {
          ...
          vault_id: VAULT_ID,
          vault_device_id: DEVICE_ID,
          issued_at: clientIssuedAt,
          expires_at: clientExpiresAt,
        },
        ownership_proof: { public_key_fingerprint: "f".repeat(64) },
      }),
    })
  );
